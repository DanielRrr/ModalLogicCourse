...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Ввели формулы {\bf AT}, {\bf A}4, {\bf AD}, {\bf ACR}, {\bf AB}
    \item Определили $p$-морфизмы шкал и моделей Крипке
    \item С помощью $p$-морфизмов показали, что иррефлексивность невыразима в модальном языке
  \end{itemize}
\end{frame}
...
\onslide<1->{
  \begin{defin}
    ...
    \onslide<2->{\begin{itemize}
      ...
      \onslide<3->{\item В $\mathcal{L}$ содержится формула ${\bf A K}$ $\Box (p \to q) \to (\Box p \to \Box q)$}
      \onslide<4->{\item $\mathcal{L}$ замкнуто относительно следующих правил:
        \begin{enumerate}
          \item [MP] $\phi \in \mathcal{L} \: \& \: \phi \to \psi \in \mathcal{L} \Rightarrow \psi \in \mathcal{L}$
          \item [Nec] $\phi \in \mathcal{L} \Rightarrow \Box \phi \in \mathcal{L}$
          \item [Sub] $\phi(p) \in \mathcal{L} \Rightarrow \phi [p := \psi]$, где $\psi \in Fm$
        \end{enumerate}
        }
    \end{itemize}
  \end{defin}
...
\begin{frame}
  ...
  \begin{theor}
    Пусть $\mathcal{F} = \langle W, R \rangle$, тогда $\operatorname{Log}(\mathcal{F})$ --- это нормальная модальная логика
  \end{theor}

\onslide<2->{
Общезначимость тавтологий и замкнутность $\operatorname{Log}(\mathcal{F})$ относительно ${\bf MP}$ очевидна. Покажем, что в произвольной шкале общезначима формула ${\bf AK}$}

\onslide<3->{
  \begin{proof}
    Пусть $\vartheta : \mathbb{V} \to 2^W$ --- это оценка и пусть
    $\mathcal{M} = \langle \mathcal{F}, \vartheta \rangle$ --- это модель на шкале $\mathcal{F}$.
    Предположим, что для
    для любого $w \in W$, если $\mathcal{M}, w \models \Box (p \to q)$ и $\mathcal{M}, w \models \Box p$.
    Тогда для любого $v \in R(w)$, $\mathcal{M}, v \models p \to q$ и $\mathcal{M}, v \models p$.
    Отсюда, $\mathcal{M}, v \models q$. Тогда и $\mathcal{M}, w \models \Box q$.}
  \end{proof}
\end{frame}
...
\onslide<1->{
Покажем, $\operatorname{Log}(\mathcal{F})$ замкнута относительно правила Nec.}
...
\onslide<1->{
  Покажем, $\operatorname{Log}(\mathcal{F})$ замкнута относительно правила Sub. Для этого введем оценку формулы $|| \phi ||_{\mathcal{M}} = \{ x \in W \: | \: \mathcal{M}, x \models \phi \}$, где $\mathcal{M}$ --- это модель.}

\onslide<2->{
\begin{proof}
  Пусть $\mathcal{F} \models \phi(p)$, $\psi$ --- это модальная формула и $\mathcal{M} = \langle \mathcal{F}, \vartheta \rangle$ --- модель. Посмотрим модель $\mathcal{M}^{'} = \langle \mathcal{F}, \vartheta' \rangle$, такую что $\vartheta'(p) = ||\psi||_{\mathcal{M}}$.} \onslide<3->{Несложной индукцией по $\phi$ можно установить следующую эквивалентность:
...
\begin{frame}
  \frametitle{Теорема корректности относительно класса шкал}

\onslide<1->{
\begin{col}
  Пусть $\mathbb{F}$ --- это класс шкал Крипке, тогда $\operatorname{Log}(\mathbb{F})$ --- это нормальная модальная логика
\end{col}
}
\onslide<2->{
\begin{proof}
  По определению, $\operatorname{Log}(\mathbb{F}) = \bigcap \limits_{\mathcal{F} \in \mathbb{F}} \operatorname{Log}(\mathcal{F})$. По теореме корректности, для каждого $\mathcal{F} \in \mathbb{F}$, $\operatorname{Log}(\mathcal{F})$ --- нормальная модальная логика. Тогда в каждом множестве $\operatorname{Log}(\mathcal{F})$ содержатся булевы тавтологии, аксиома нормальности, и оно замкнуто относительно трех правил.}
\end{proof}
\end{frame}
...
\onslide<1->{
  \begin{defin}
    Нормальная модальная логика $\mathcal{L}$ полна по Крипке, если она является логикой некоторого класса шкал. То есть, существует класс $\mathbb{F}$, такой, что
    $\mathcal{L} = \operatorname{Log}(\mathbb{F})$.
  \end{defin}
}
\onslide<2->{
  Следующая цель: теорема о полноте для минимальной нормальной модальной логики
  \begin{theor}
    Пусть $\mathbb{F}$ --- это класс всех шкал Крипке, тогда ${\bf K} = \operatorname{Log}(\mathbb{F})$.
  \end{theor}
}
\end{frame}
...
\onslide<1->{
  \begin{defin}
    ...
    $\phi_1, \dots, \phi_n \in \Gamma$, такие, что $\neg \bigwedge \limits_{i = 1}^n \phi_i \in \mathcal{L}$. В противном случае,
...
\onslide<3->{
  \begin{proof}
    ...
    $A = \bigwedge \limits_{i = 1}^n \phi_i \land \bigwedge \limits_{j = 1}^m \psi_j$.} \onslide<6->{Тогда $\mathcal{L} \vdash \neg (A \land \phi)$ и
...
\onslide<1->{
  \begin{lem}
    ...
    \end{itemize}
  \end{lem}
\onslide<2->{Разберем для примера пункт 4.}
\onslide<3->{\begin{proof}
    ($\Rightarrow$) Пусть $\phi \lor \psi \in \Gamma$ и при этом $\phi \notin \Gamma$ и $\psi \notin \Gamma$.
    Тогда $\neg \phi \in \Gamma$ и $\neg \psi \in \Gamma$.
    Тогда $\Gamma$ противоречиво, ибо $\neg ((\neg \phi \land \neg \psi) \land (\phi \lor \psi)) \in \mathcal{L}$, что тавтология, и при этом $(\neg \phi \land \neg \psi) \land (\phi \lor \psi) \in \Gamma$.}
  \end{proof}
\end{frame}
...
\begin{frame}
  ...
  \begin{lem}
    ...
    \end{itemize}
  \end{lem}
    Разберем для примера пункт 4.
  \begin{proof}
    \onslide<2->{($\Leftarrow$) Пусть $\phi \in \Gamma$ или $\psi \in \Gamma$, при этом $\phi \lor \psi \notin \Gamma$.
    Тогда $\neg (\phi \lor \psi) \in \Gamma$. Откуда $\phi \land \neg (\phi \lor \psi) \in \Gamma$. С другой стороны, $\neg (\phi \land \neg (\phi \lor \psi)) \in \mathcal{L}$, будучи тавтологией.}
  \end{proof}
\end{frame}
...
\onslide<1->{
  \begin{defin} Пусть $\mathcal{L}$ --- это нормальная модальная логика.} \onslide<2->{Канонической шкалой логики $\mathcal{L}$ называется шкала
    ...
    \onslide<4->{\item $\Gamma R_{\mathcal{L}} \Delta \Leftrightarrow_{def} \forall \phi \:\: \Box \phi \in \Gamma \Rightarrow \psi \in \Delta$}
    \end{itemize}
  \end{defin}
...
\onslide<5->{
  \begin{defin}
    ...
    \begin{center}
      $\vartheta_{\mathcal{L}}(p) =_{def} \{ \Gamma \in W_{\mathcal{L}} \: | \: p \in \Gamma \}$
    \end{center}
  \end{defin}
}
\end{frame}
...
\onslide<3->{
  \begin{proof}
    ...
    \onslide<6->{Положим $\Psi = \{ \theta \: | \: \Box \theta \in \Gamma \} \cup \{ \neg \psi \}$.} \onslide<7->{Покажем, что $\Psi$ непротиворечиво.}
    \onslide<8->{Предположим противное.}
    \onslide<9->{Тогда существуют $\theta_1, \dots, \theta_n$, такие что $\neg (\land_{i=1}^{n} \theta_i \land \neg \psi) \in \mathcal{L}$, где $\theta_i$ такие, что $\Box \theta_i \in \Gamma$.}
...
\onslide<2->{
  \begin{proof}
    $\begin{array}{lll}
    & \mathcal{L} \vdash \neg (\land_{i=1}^{n} \theta_i \land \neg \psi) & \\}
    \onslide<3->{ & \:\:\:\: \text{Эквивалентность из классической логики} & \\
    & \mathcal{L} \vdash \land_{i=1}^{n} \theta_i \to \psi & \\}
    \onslide<4->{& \:\:\:\: \text{{\bf Nec} + {\bf MP} + \text{ пример вывода в ${\bf K}$}} & \\
    & \mathcal{L} \vdash \land_{i=1}^{n} \Box \theta_i \to \Box \psi & \\}
    \end{array}$

    \onslide<5->{
      Ясно, что $\land_{i=1}^{n} \Box \theta_i \to \Box \psi \in \Gamma$, так как $\land_{i=1}^{n} \Box \theta_i \in \Gamma$ по лемме о максимальных непротиворечивых
      множествах. Тогда $\Box \psi \in \Gamma$. Противоречие.}
  \end{proof}
\end{frame}

\begin{frame}
  \frametitle{Теорема о канонической модели}

\onslide<1->{
  \begin{theor}
    $\phi \in \mathcal{L} \Leftrightarrow \mathcal{M}_{\mathcal{L}} \models \phi$
  \end{theor}
}
...
\onslide<2->{
  \begin{proof}
    \begin{enumerate}
    \item $(\Rightarrow)$ Если $\phi \in \mathcal{L}$, тогда для каждого максимального $\mathcal{L}$-непротиворечивого $\Gamma$, $\phi \in \Gamma$.
    По лемме о канонической модели, $\mathcal{M}_{\mathcal{L}}, \Gamma \models \phi$. Тогда $\mathcal{M}_{\mathcal{L}} \models \phi$.}
\onslide<3->{
    \item $(\Leftarrow)$ Пусть $\phi \notin \mathcal{L}$, тогда $\{ \neg \phi \}$ непротиворечиво.} \onslide<4->{Тогда, по лемме Линденбаума, найдется максимальное $\mathcal{L}$-непротиворечивое множество $\Gamma$, что $\{ \neg \phi \} \subseteq \Gamma$.}
    \onslide<5->{Тогда $\neg \phi \in \Gamma$.} \onslide<6->{По лемме о канонической модели,
    $\mathcal{M}_{\mathcal{L}}, \Gamma \models \neg \phi$.} \onslide<7->{Тогда $\mathcal{M}_{\mathcal{L}}, \Gamma \nvDash \phi$.}
  \end{enumerate}
  \end{proof}
\end{frame}
...
\onslide<1->{
  \begin{col}
    $\operatorname{Log}(\mathcal{F}_{\mathcal{L}}) \subseteq \mathcal{L}$
  \end{col}
}

\onslide<2->{
  \begin{theor}
      ${\bf K} = \operatorname{Log}(\mathcal{F})$, где $\mathcal{F}$ --- это класс всех шкал Крипке.
  \end{theor}
}
...
\onslide<3->{
  \begin{proof} Рассмотрим следующую цепочку включений:
    ...
    ${\bf K} \subseteq \operatorname{Log}(\mathbb{F}) \subseteq \operatorname{Log}(\mathcal{F}_{\bf K}) \subseteq {\bf K}$
    }
...
\onslide<1->{
  \begin{enumerate}
    ...
    \onslide<2->{\item Дадим понятие канонической логики и рассмотрим примеры канонических логик.}
    \onslide<3->{\item Обсудим подробнее логику Г\"{е}деля-Л\"{е}ба ${\bf GL}$, ее связь с теорией доказательств, ее неканоничность и покажем, что она полна по Крипке.}
  \end{enumerate}
\end{frame}
